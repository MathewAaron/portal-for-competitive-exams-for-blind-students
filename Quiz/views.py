...
def question_xls_upload(request):
    CourseName=forms.Coursename()
    if request.method == "GET":
        return render(request,'quiz/ques_upload.html',{'CourseName':CourseName})

    if request.method == "POST":
        CourseName=forms.Coursename(request.POST)
        if CourseName.is_valid():
            course=models.Course.objects.get(id=request.POST.get('courseID'))
            print(course)
            print(CourseName)
            question_resource = QuestionResource()
            dataset= Dataset()
            new_question = request.FILES['xlsxfile']
            if not new_question.name.endswith('xlsx'):
                messages.info(request,"Please select file with xlxs format")
                return render(request,'ques_upload.html')
            imported_data = dataset.load(new_question.read(),format='xlsx')
...
def update_question_view(request,pk):
    question=models.Question.objects.get(id=pk)
    print(question.course,question.question,question.marks,question.option1,question.option2,question.option3,question.option4,question.answer)
    questionForm = forms.QuestionForm()
    if request.method=='POST':
        questionForm=forms.QuestionForm(request.POST)
        ans = questionForm['answer'].value()
        print(ans)
        if (ans!= None):
    ...
    return render(request,'quiz/admin_update_question.html',{'questionForm':questionForm,'questionselected':question})
